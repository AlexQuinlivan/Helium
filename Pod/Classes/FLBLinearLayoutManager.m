...
-(void) measureVertical:(UIView *) view
    ...
    FLBMeasureSpecMode widthMode = [FLBLayout measureSpecMode:widthMeasureSpec];
    FLBMeasureSpecMode heightMode = [FLBLayout measureSpecMode:heightMeasureSpec];
    
    BOOL matchWidth = NO;
    
    NSInteger baselineChildIndex = view.flb_baselineAlignedChildIndex;
    ...
    for (int i = 0; i < subviews.count; i++) {
        ...
        if (i < baselineChildIndex && childWeight > 0) {
            @throw [NSException exceptionWithName:@"FLBLinearLayoutManagerMeasureException"
                                           reason:
        ...
        if (widthMode != FLBMeasureSpecExactly && childLayoutWidth == FLBLayoutParamMatch) {
            matchWidth = true;
        ...
        allFillParent = allFillParent && childLayoutWidth == FLBLayoutParamMatch;
        if (childWeight > 0) {
    ...
    heightSize = [FLBLayout resolveSize:heightSize spec:heightMeasureSpec];
    int32_t delta = heightSize - self.totalLength;
    if (delta != 0 && totalWeight > 0.0f) {
        CGFloat weightSum = view.flb_weightSum > 0.0f ?: totalWeight;
        self.totalLength = 0;
        for (int i = 0; i < subviews.count; i++) {
            UIView* child = subviews[i];
            if (child.isHidden) {
            ...
            if (childExtra > 0) {
                ...
                delta -= share;
                FLBMeasureSpec childWidthMeasureSpec = [FLBLayout childMeasureSpec:widthMeasureSpec
                                                                           padding:child.paddingLeft + child.paddingRight + child.marginLeft + child.marginRight
                                                                         dimension:childLayoutWidth];
                if ((childLayoutHeight != 0) || (heightMode != FLBMeasureSpecExactly)) {
                    int childHeight = child.measuredHeight + share;
                    if (childHeight < 0) {
                        childHeight = 0;
                    }
                    [FLBLayout measureView:child
                                 widthSpec:childWidthMeasureSpec
                                heightSpec:[FLBLayout measureSpecWithSize:childHeight mode:FLBMeasureSpecExactly]];
                } else {
                    [FLBLayout measureView:child
                                 widthSpec:childWidthMeasureSpec
                                heightSpec:[FLBLayout measureSpecWithSize:(share > 0) ? share : 0 mode:FLBMeasureSpecExactly]];
                }
            }
            ...
            int32_t measuredWidth = child.measuredWidth + margin;
            maxWidth = MAX(maxWidth, measuredWidth);
            BOOL matchWidthLocally = widthMode != FLBMeasureSpecExactly &&
            childLayoutWidth == FLBLayoutParamMatch;
    ...
    if (!allFillParent && widthMode != FLBMeasureSpecExactly) {
...
-(void) layoutVertical:(UIView *) view
    ...
    FLBGravity gravity = view.layoutGravity;
    FLBGravity minorGravity = gravity & FLBGravityHorizontalMask;
    FLBGravity majorGravity = gravity & FLBGravityVerticalMask;
    if (majorGravity != FLBGravityTop) {
        switch (majorGravity) {
    ...
    for (int i = 0; i < subviews.count; i++) {
        ...
        switch (gravity & FLBGravityHorizontalMask) {
            case FLBGravityLeft:
        ...
        [FLBLayout setChild:child frame:CGRectMake(childLeft, childTop, childWidth, childHeight)];
...
-(void) forceUniformWidth:(UIView *) view
        ...
        heightMeasureSpec:(FLBMeasureSpec) heightMeasureSpec {
    FLBMeasureSpec uniformMeasureSpec = [FLBLayout measureSpecWithSize:view.measuredWidth mode:FLBMeasureSpecExactly];
    NSArray* subviews = view.subviews;
    for (int i = 0; i < count; i++) {
        UIView* child = subviews[i];
        if (!child.isHidden) {
            if (child.layoutWidth == FLBLayoutParamMatch) {
